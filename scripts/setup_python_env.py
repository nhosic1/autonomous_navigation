...
def is_venv(dir_path):
    ...
    return dir_path.is_dir() and (
        (dir_path / 'Scripts' / 'Activate.ps1').is_file() or
        (dir_path / 'bin' / 'activate').is_file() 
    )
...
def install_dependencies_in_venv(venv_path, requirements_file_path):
    ...
    # Install dependencies from sphinx_requirements.txt
...
if __name__ == "__main__":
    ...
    install_dependencies_in_venv(venv_path, "sphinx_requirements.txt")
